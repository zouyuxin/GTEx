...
m_V3_genewide = readRDS('output/GTExV6/m_V3_genewide_post.rds')
...
```{r loglikelihood}
logliks = c(get_loglik(m_ignore), get_loglik(m_simple), get_loglik(m_current), get_loglik(m_V3_genewide))
logliks_EZ = c(get_loglik(m_ignore_EZ), get_loglik(m_simple_EZ), get_loglik(m_current_EZ), get_loglik(m_V3_EZ_genewide))
...
row.names(tmp) = c('Ignore', 'Simple', 'Current', 'V3 genewide', 'V3 Current genewide', 'V3 Current tissuewide')
...
barplot(get_estimated_pi(m_V3_genewide), las=2, cex.names = 0.7, main = 'V3 genewide EE')
...
barplot(get_estimated_pi(m_V3_EZ_Current_genewide), las=2, cex.names = 0.7, main = 'V3 Current genewide EZ')
barplot(get_estimated_pi(m_V3_EZ_Current_tissuewide), las=2, cex.names = 0.7, main = 'V3 Current tissuewide EZ')
```
...
numsig_EE = c(length(get_significant_results(m_ignore)), 
              ...
              length(get_significant_results(m_V3_genewide)))
numsig_EZ = c(length(get_significant_results(m_ignore_EZ)), 
              ...
              length(get_significant_results(m_V3_EZ_genewide)))
...
stronggeneV3Current = data.frame(m_V3_EZ_Current_tissuewide$result$PosteriorMean[13837,])
colnames(stronggeneV3Current) = 'EffectSize'
stronggeneV3Current$Group = row.names(stronggeneV3Current)
stronggeneV3Current$se = m_V3_EZ_Current_tissuewide$result$PosteriorSD[13837,]
p4 = ggplot(stronggeneV3Current, aes(y = EffectSize, x = Group)) + 
  geom_point(show.legend = FALSE, color=gtex.colors) + ylim(c(-1.3,1.1)) + coord_flip() + ggtitle('ENSG00000249898 V3 Current tissuewide') + 
  geom_errorbar(aes(ymin=EffectSize-1.96*se, ymax=EffectSize+1.96*se), width=0.4, show.legend = FALSE, color=gtex.colors) + 
  theme_bw(base_size=12) + theme(axis.text.y = element_text(colour = gtex.colors, size = 6))
grid.arrange(p1, p2, p3, p4, nrow = 2)
```

The gene significant in `V3 EZ Current tissuewide`, not in `current EZ`:
```{r fig.align='center', fig.height=15,fig.width=15}
ind = setdiff(get_significant_results(m_V3_EZ_Current_tissuewide), get_significant_results(m_current_EZ))[100]
...
p1 = ggplot(stronggene, aes(y = EffectSize, x = Group)) + 
  geom_point(show.legend = FALSE, color=gtex.colors) + coord_flip() + ggtitle('ENSG00000092199') + ylim(c(-1.3,1.1)) + geom_errorbar(aes(ymin=EffectSize-1.96*se, ymax=EffectSize+1.96*se), width=0.4, show.legend = FALSE, color=gtex.colors) + 
  theme_bw(base_size=12) + theme(axis.text.y = element_text(colour = gtex.colors, size = 6))
...
p2 = ggplot(stronggeneSimple, aes(y = EffectSize, x = Group)) + 
  geom_point(show.legend = FALSE, color=gtex.colors) + ylim(c(-1.3,1.1)) + coord_flip() + ggtitle('ENSG00000092199 Simple') + 
  geom_errorbar(aes(ymin=EffectSize-1.96*se, ymax=EffectSize+1.96*se), width=0.4, show.legend = FALSE, color=gtex.colors) + 
  theme_bw(base_size=12) + theme(axis.text.y = element_text(colour = gtex.colors, size = 6))
...
p3 = ggplot(stronggeneCurrent, aes(y = EffectSize, x = Group)) + 
  geom_point(show.legend = FALSE, color=gtex.colors) + coord_flip() + ggtitle('ENSG00000092199 Current') + ylim(c(-1.3,1.1)) + 
  geom_errorbar(aes(ymin=EffectSize-1.96*se, ymax=EffectSize+1.96*se), width=0.4, show.legend = FALSE, color=gtex.colors) + 
  theme_bw(base_size=12) + theme(axis.text.y = element_text(colour = gtex.colors, size = 6))

stronggeneV3Current = data.frame(m_V3_EZ_Current_tissuewide$result$PosteriorMean[ind,])
colnames(stronggeneV3Current) = 'EffectSize'
stronggeneV3Current$Group = row.names(stronggeneV3Current)
stronggeneV3Current$se = m_V3_EZ_Current_tissuewide$result$PosteriorSD[ind,]
p4 = ggplot(stronggeneV3Current, aes(y = EffectSize, x = Group)) + 
  geom_point(show.legend = FALSE, color=gtex.colors) + ylim(c(-1.3,1.1)) + coord_flip() + ggtitle('ENSG00000092199 V3 Current tissuewide') + 
  geom_errorbar(aes(ymin=EffectSize-1.96*se, ymax=EffectSize+1.96*se), width=0.4, show.legend = FALSE, color=gtex.colors) + 
  theme_bw(base_size=12) + theme(axis.text.y = element_text(colour = gtex.colors, size = 6))
...
```{r fig.align='center',fig.height=15,fig.width=30}
par(mfrow=c(1,2))
x           <- get_pairwise_sharing(m_V3_EZ_Current_genewide)
colnames(x) <- colnames(get_lfsr(m_V3_EZ_Current_genewide))
rownames(x) <- colnames(x)
...
x           <- get_pairwise_sharing(m_V3_EZ_Current_tissuewide)
colnames(x) <- colnames(get_lfsr(m_V3_EZ_Current_tissuewide))
rownames(x) <- colnames(x)
corrplot::corrplot(x, method='color', type='upper', tl.col="black", tl.srt=45, tl.cex = 0.7, diag = FALSE, col=clrs, cl.lim = c(0,1), title = 'V3 EZ Current tissuewide', mar=c(0,0,5,0))
```
